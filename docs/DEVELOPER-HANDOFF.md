...
| **D31** | **High** | The AI consensus engine ran on every scan and its analysis was **discarded**. The `store` job wrote `consensus: {status: "success"}` and nothing else, while the dashboard's AI panel looks for `ai_consensus`, found nothing, and hid itself. Four providers were being paid to analyse every finding, for nobody | **Fixed** — `dnsguard/consensus.py` folds it in, vendor names stripped |
| **D32** | **High** | `_quick_wins` read only `evidence["remediation"]`, never `Finding.remediation`. A real report's action list offered the client the low-severity DNSSEC item and **omitted the critical subdomain takeover** — because `alias_takeover` uses the field `base.py` documents and the older module does not. Writing a module correctly excluded it from the client's to-do list | **Fixed** — field first, legacy key as fallback, matching the D18 correction |
| **D29** | **High** | The client-facing summary said **"Overall risk is low (27/100)"** on a report carrying a confirmed critical subdomain takeover. Risk was dominated by email posture — a critical adds 15 points, not enough to leave the "low" band — and the summary led with the email grade in every case, so the worst finding never reached the sentence a client reads | **Fixed** — overall risk is never better than the worst finding, and the summary leads with what is urgent |
| **D30** | Med | The same dangling alias was reported twice: `alias_takeover` at critical/confirmed, `subdomain_discovery` at high with "if the destination is a de-provisioned hosting account". Two modules, two severities, one problem, the weaker wording undermining the stronger | **Fixed** — only the module that verifies it reports it; a test keeps the two modules in the same groups |
...
| D32 found by reading a real report's action list | Run `34227353730`'s report: `quick_wins` contained only "Enable DNSSEC…"; after the fix the list leads with "Delete the alias record for vpn.ironcityit.com…" | 2026-09-10 |
| Each AI analysis is labelled with the finding it is about | Run `34216984420`'s artifacts re-merged: 8 analyses, each carrying `finding_fingerprint`/`finding_title`/`finding_severity`. The engine's own severity sits beside ours — `AI MEDIUM vs ours low` on DNSSEC | 2026-09-08 |
| The consensus payload is far inside Firestore's document limit | Enriched payload for a real scan: **41KB compact** against a 1 MiB limit. Stripping `model_responses` took the analysis from 204KB to 19KB | 2026-09-08 |
